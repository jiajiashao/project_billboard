...
class MoondreamBoxPromptor:
    ...
    def predict_topk(self, frame_bgr, k: int = 3) -> List[BoxPrediction]:
        """Return up to k BoxPrediction across all prompts, sorted by score desc.
        Uses the same geometry/score gating as __call__.
        """
        try:
            import numpy as np  # type: ignore
            import cv2  # type: ignore
        ...
        for term in self.prompts:
            ...
            for obj in objects:
                try:
                    x0n = float(obj.get("x_min", 0.0)); y0n = float(obj.get("y_min", 0.0))
                    x1n = float(obj.get("x_max", 0.0)); y1n = float(obj.get("y_max", 0.0))
                    score = float(obj.get("score", 0.5))
                except Exception:
                    continue
                x0n = max(0.0, min(1.0, x0n)); y0n = max(0.0, min(1.0, y0n))
                x1n = max(0.0, min(1.0, x1n)); y1n = max(0.0, min(1.0, y1n))
                x0 = x0n * W; y0 = y0n * H; x1 = x1n * W; y1 = y1n * H
                if not (x1 > x0 and y1 > y0):
                    continue
                bw = x1 - x0; bh = y1 - y0
                if not (0.01 * W <= bw <= 0.95 * W and 0.01 * H <= bh <= 0.95 * H):
                ...
                if score < self.threshold:
                    continue
                preds.append(BoxPrediction(label=term, score=score, x0=x0, y0=y0, x1=x1, y1=y1))
        # Sort by score desc and return top-k
        preds.sort(key=lambda p: float(getattr(p, 'score', 0.0)), reverse=True)
        return preds[: max(1, int(k))]
