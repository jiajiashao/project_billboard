...
def parse_args() -> base.argparse.Namespace:
    ...
    # YOLO autoprompt
    parser.add_argument("--auto-prompt-yolo", action="store_true", default=False)
    parser.add_argument("--yolo-model", default="runs/detect/train2/weights/best.pt")
...
def process_clip_owlvit(
    ...
    use_yolo = bool(getattr(args, "auto_prompt_yolo", False))
    yolo_promptor = None
    promptor = None
    if use_yolo:
        try:
            yolo_promptor = YoloBoxPromptor(model_path=args.yolo_model, device=str(getattr(args, "yolo_device", "0")), conf=float(args.yolo_conf_thr), imgsz=1280)
        except Exception:
            yolo_promptor = None
        if callable(log):
            try:
                log(f"Autoprompt[YOLO] model={args.yolo_model} conf={args.yolo_conf_thr} device={getattr(args, "yolo_device", "0")}")
            except Exception:
                pass
    else:
        if args.auto_prompt:
            try:
                promptor = OwlVitBoxPromptor(model_id=args.owlvit_model, device=run_dev, prompts=prompts, score_thr=float(args.owlvit_score_thr), nms_iou=0.5)
            except Exception:
                promptor = None
        if callable(log):
            try:
                log(f"Autoprompt[OWL-ViT] model={args.owlvit_model} score_thr={args.owlvit_score_thr} device={run_dev}")
                log("Prompts used: " + ", ".Join(", ", $null))
            except Exception:
                pass
    if callable(log):
        try:
    ...
    with base.torch.inference_mode():
        for i, (s, e) in enumerate(shot_bounds, start=1):
            ...
            # Run autoprompt on global start frame
            preds = []
            if shot_frames:
                bgr0 = _to_bgr(frames[s])
                if bgr0 is not None:
                    try:
                        if use_yolo and yolo_promptor is not None:
                            preds = yolo_promptor(bgr0, max_det=int(getattr(args, 'yolo_max_objects', 3))) or []
                        elif (not use_yolo) and promptor is not None:
                            preds = promptor.predict_topk(bgr0, k=3) or []
                    except Exception:
                        preds = []

            # Fallback to GT if requested and no OWL-ViT detections
            fb_box = None
            ...
            if preds:
                for p in preds[:3]:
                    ...
                    try:
                        ...
                        scores.append(0.0)
                mode = ("yolo" if use_yolo else "owlvit")
            else:
            ...
            if mode == "owlvit":
                log(f"Shot {i}/{len(shot_bounds)}: owlvit ok (n={len(boxes)}) - seeding @ {s}")
            else:
                log(f"Shot {i}/{len(shot_bounds)}: {mode} ok (n={len(boxes)}) - seeding @ {s}")
            try:
