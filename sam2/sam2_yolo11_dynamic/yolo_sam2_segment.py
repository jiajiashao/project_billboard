...
def parse_args() -> argparse.Namespace:
    ...
    # YOLO
    p.add_argument("--yolo-model", required=True, help="Path to YOLO weights (e.g., runs/detect/trainN/weights/best.pt)")
    ...
    # SAM-2
    p.add_argument("--sam2-weights", default="models/sam2.1-hiera-tiny", help="HuggingFace weights for SAM-2 video")
    p.add_argument("--input-width", type=int, default=1280, help="Target decode width (frames resized keeping aspect)")
    # Device/Perf
    p.add_argument("--device", choices=["cpu", "cuda", "mps"], default=None, help="Run device for both models")
    ...
    # GT root
    p.add_argument("--gt-root", default="D:\\Billboard_Project\\sam2\\data\\gt_frames")
    return p.parse_args()
